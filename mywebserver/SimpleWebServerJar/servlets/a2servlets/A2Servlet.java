...
public class A2Servlet extends HttpServlet { 
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException { 
        ...
        if("Checkout".equals(action)) {
            boolean success = handleCheckout(req, res);
            System.out.println("In A2Servlet.if(Checkout), success=" + success);
        }
        if("Clear".equals(action)) { 
            boolean success = handleClear(req, res);
            System.out.println("In A2Servlet.if(clear), success=" + success);
        }
        if("Submit Cart".equals(action)) { 
            ...
            if(success){
               handleOrderComplete(req, res);
            }else{
                res.sendRedirect("/a2html/home.html");
            }
        }
        if("checkorder".equals(action)) { 
            boolean success = handleCheckOrder(req, res);
            System.out.println("In A2Servlet.if(checkorder), success=" + success);
        }
    }
    ...
    public void handleOrderComplete(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    public boolean handleClear(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        System.out.println("In A2Servlet.handleClear()");
    ...
    public boolean handleCheckout(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        boolean success = false;
        System.out.println("In A2Servlet.handleCheckout()");
    ...
    public boolean handleCheckOrder(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        boolean success = false;
        String fUsername, fOrderNumber, fDeadlines, fCaffeine, fQuill, fPlanner, fCloak, fTotal, fName, fAddress, fOrders;
        String enteredUserName = req.getParameter("username");
        String enteredOrderNumber = req.getParameter("ordernumber");
        String filename = enteredUserName + enteredOrderNumber + ".txt";
        System.out.println("Searching for file: " + filename);
        File f = new File(filename);
        if(f.exists()) {
            try (BufferedReader br = new BufferedReader(new FileReader(f))){
                fUsername = br.readLine();
                fOrderNumber = br.readLine();
                fDeadlines = br.readLine();
                fCaffeine = br.readLine();
                fQuill = br.readLine();
                fPlanner = br.readLine();
                fCloak = br.readLine();
                fTotal = br.readLine();
                fName = br.readLine();
                ...
                out.println("<body>");
                out.println("<h1 style=\"text-align: center;\">Order Summary for Order Number" + fOrderNumber + "</h1>");
                out.println("<p>Thank you for your order, " + fName + ". Your order totaling $" + fTotal + " will be shipped to " + fAddress + " soon. Thank you for your patience. </p>");
                out.println("<p><b>Order Summary:</b> Deadline Defenders: " + fDeadlines + " @ $49.99ea; Caffeine Conductor's Kits: " + fCaffeine + " @ $79.99ea; Reviewer's Rebuttal Quills: " + fQuill + " @ $39.99ea; Procrastinator's Paradise Planners: " + fPlanner + "@ $29.99ea; Conference Conjurer's Cloaks: " + fCloak + " @ $199.99ea.</p>");
                out.println("<p>To return to the landing page, click <a href=\"/a2html/home.html\">here</a></p>");
                out.println("</body></html>");
                out.flush();
                success = true;
            ...
            out.println("<!DOCTYPE html>");
            out.println("<html>");
            out.println("<head><title>ERROR: Order Not Found</title></head>");
            out.println("<body>");
            out.println("<h1 style=\"text-align: center; color: red;\">Warning!!!</h1>");
            out.println("<h3 style=\"text-align: center;\">Your order was not found basd on the provided information.</h3>");
            out.println("<h3 style=\"text-align: center;\">Please use your browsers back button to navigate back to the home page and try again.</h3>");
            out.println("</body></html>");
            out.flush();
        }
        return success;
